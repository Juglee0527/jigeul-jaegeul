...
기본 3분/6분/10분 또는 퀵 2분/5분 도달 → 단계별 보스 생성 → 타이머 정지 → 보스 처치 → 전설 보물상자 → UpgradeScene
...
`GameSession.gameLength`는 `standard`와 `quick` 중 하나다. 기본 모드는 3·6·10분 보스 세 개를, 퀵 모드는 2·5분 보스 두 개를 예약한다. 보스전에서는 생존 타이머와 별개로 계속 증가하는 전투 시계를 사용한다. 자동 공격 쿨다운, 피격 무적시간, 보스 이동과 패턴 예약은 전투 시계를 참조하므로 예약 시각에 생존 타이머가 정지해도 전투가 멈추지 않는다. 보스전에는 일반 적을 추가 생성하지 않는다. `Enemy`는 대시 이동 오버라이드, 순간이동, 거리 유지와 공전 이동을 지원한다. `BossProjectile`은 조준·원형·회전 탄막을 공통 처리하며 각 공격은 Phaser Tween 경고를 먼저 표시한다.
...
메인 메뉴는 위·아래 방향키로 포커스를 순환하고 Enter로 선택하며, 좌·우 방향키로 난이도를 변경한다. 방어력은 개별 피해의 최대 90%까지만 감소시키고 최소 1 피해를 보장한다. 자동 회복은 마지막 피격 1초 뒤 재개하며 최대 멘탈의 초당 10%를 상한으로 둔다. 체력이 0인 플레이어는 전설 부활 횟수가 남은 경우에만 45% 멘탈로 복귀하고, 그 외에는 즉시 게임 종료를 확정한다. HUD 경험치는 `현재 / 필요 (정수%)` 형식으로 표시한다.
